use dioxus::prelude::*;

use crate::shared::ui::icon::{IconConfig, IconProps, icon};

#[component]
pub fn Discord(props: IconProps) -> Element {
    icon(
        IconConfig {
            name: "discord",
            view_box: "0 0 24 24",
            width: 24,
            height: 24,
        },
        props,
        rsx! {
            path {
                d: "M5.5 16C10.5 18.5 13.5 18.5 18.5 16",
                fill: "none",
                stroke: "currentColor",
                stroke_width: "1.5",
                stroke_linecap: "round",
                stroke_linejoin: "round",
            }
            path {
                d: "M15.5 17.5L16.5 19.5C16.5 19.5 20.6713 18.1717 22 16C22 15 22.5301 7.85339 19 5.5C17.5 4.5 15 4 15 4L14 6H12",
                fill: "none",
                stroke: "currentColor",
                stroke_width: "1.5",
                stroke_linecap: "round",
                stroke_linejoin: "round",
            }
            path {
                d: "M8.52828 17.5L7.52828 19.5C7.52828 19.5 3.35695 18.1717 2.02828 16C2.02828 15 1.49819 7.85339 5.02828 5.5C6.52828 4.5 9.02828 4 9.02828 4L10.0283 6H12.0283",
                fill: "none",
                stroke: "currentColor",
                stroke_width: "1.5",
                stroke_linecap: "round",
                stroke_linejoin: "round",
            },
            path {
                d: "M8.5 14C7.67157 14 7 13.1046 7 12C7 10.8954 7.67157 10 8.5 10C9.32843 10 10 10.8954 10 12C10 13.1046 9.32843 14 8.5 14Z",
                fill: "none",
                stroke: "currentColor",
                stroke_width: "1.5",
                stroke_linecap: "round",
                stroke_linejoin: "round",
            },
            path {
                d: "M15.5 14C14.6716 14 14 13.1046 14 12C14 10.8954 14.6716 10 15.5 10C16.3284 10 17 10.8954 17 12C17 13.1046 16.3284 14 15.5 14Z",
                fill: "none",
                stroke: "currentColor",
                stroke_width: "1.5",
                stroke_linecap: "round",
                stroke_linejoin: "round",
            }
        },
    )
}
